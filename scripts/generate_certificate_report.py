...
def report_row(cert_path: Path, circuit_path: Path | None = None) -> dict:
    ...
    verify_ok, messages = verify_certificate(cert, circuit_path)
    if not schema_ok:
    ...
    if circuit_path is not None:
        circuit_ok = _message_present(messages, "public circuit hash and resource counts verified")
        circuit_hash_verified = str(circuit_ok).lower()
        resource_counts_verified = str(circuit_ok).lower()
    ...
    status = "pass" if schema_ok and verify_ok else "fail"
    ...
    return {
        ...
        "transcript_hash_verified": str(_message_present(messages, "transcript hash verified")).lower(),
...
def generate_report(rows: list[dict], md_path: Path, csv_path: Path) -> None:
    ...
    markdown += "It summarizes schema validation, transcript checks, transcript hashes, and optional public toy gate-list verification.\n\n"
